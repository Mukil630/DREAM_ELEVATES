...
(function () {
  ...
  // State
  let currentUser = JSON.parse(localStorage.getItem('dreamelevate_user')) || null;
    ...
    const htmlMarkup = `
      ...
      <div id="rewards-drawer" class="rewards-drawer">
        <div class="rewards-drawer-header">
          <h2 id="rewards-drawer-title">🍰 Elevate Club</h2>
  ...
  async function refreshUserData() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        localStorage.setItem('dreamelevate_user', JSON.stringify(freshData));
        
        // Custom event for order auto-fill page compatibility
        window.dispatchEvent(new CustomEvent('dreamelevate_user_updated', { detail: freshData }));
        syncHeaderLink();
      }
    } catch (e) {
  ...
  function renderLoginForm(container) {
    container.innerHTML = `
      <div style="padding-top: 10px;">
        <p style="margin-bottom: 24px; font-size: 0.9rem; line-height: 1.5;">
          Join the <strong>Elevate Club</strong> to claim daily points, unlock exclusive custom discount coupons, and track your baking orders!
        </p>
    ...
    document.getElementById('rewards-login-form').addEventListener('submit', async (e) => {
      ...
      try {
        ...
        if (res.ok) {
          currentUser = data;
          localStorage.setItem('dreamelevate_user', JSON.stringify(data));
          showDrawerToast('Welcome back to Elevate Club!');
          window.dispatchEvent(new CustomEvent('dreamelevate_user_updated', { detail: data }));
          syncHeaderLink();
          renderDrawerContent();
        } else {
    ...
    document.getElementById('rewards-register-form').addEventListener('submit', async (e) => {
      ...
      try {
        ...
        if (res.ok) {
          currentUser = data;
          localStorage.setItem('dreamelevate_user', JSON.stringify(data));
          showDrawerToast('Account created! Welcome to Elevate Club!');
          window.dispatchEvent(new CustomEvent('dreamelevate_user_updated', { detail: data }));
          syncHeaderLink();
          renderDrawerContent();
        } else {
  ...
  function renderDashboard(container) {
    ...
    container.innerHTML = `
      <div style="padding-top: 5px;">
        ...
        <!-- Logout Button -->
        <button id="rewards-logout-btn" class="rewards-btn" style="background:#6e6458; opacity:0.8; margin-top:30px;">Log Out of Elevate Club</button>
      </div>
    `;
      ...
      claimBtn.addEventListener('click', async () => {
        ...
        try {
          ...
          if (res.ok) {
            ...
            localStorage.setItem('dreamelevate_user', JSON.stringify(currentUser));
            showDrawerToast(`+${data.pointsEarned} Points Claimed! Streak: Day ${data.streakDay}`);
            
            // Fire confetti!
            triggerConfetti();
            
            window.dispatchEvent(new CustomEvent('dreamelevate_user_updated', { detail: currentUser }));
            syncHeaderLink();
    ...
    document.getElementById('rewards-logout-btn').addEventListener('click', () => {
      currentUser = null;
      localStorage.removeItem('dreamelevate_user');
      showDrawerToast('Logged out of Elevate Club');
      window.dispatchEvent(new CustomEvent('dreamelevate_user_updated', { detail: null }));
      syncHeaderLink();
      renderDrawerContent();
    });
  }
